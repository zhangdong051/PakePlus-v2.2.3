...
window.addEventListener("gamepadconnected", function (e) {
    ...
    document.getElementById("gamepad-status").innerText = "手柄已连接: " + e.gamepad.id;
    startPolling();
});

window.addEventListener("gamepaddisconnected", function (e) {
    document.getElementById("gamepad-status").innerText = "手柄已断开";
...
function checkAxes(currentGamepad) {
    ...
    currentGamepad.axes.forEach((axis, index) => {
        ...
        if (delta > AXIS_THRESHOLD) {
            ...
            document.getElementById('left-joystick-x').textContent = rc2;
            document.getElementById('left-joystick-y').textContent = rc3;
            document.getElementById('right-joystick-x').textContent = rc0;
            document.getElementById('right-joystick-y').textContent = rc1;
            GAME_command(); // 即时更新一次
            statusChanged = true;
        }
    });
